...
- **Manual proper-noun blocklist.** `--exclude-nouns-file` takes a plain text file, one lemma per line (case-insensitive; blank lines and `#`-prefixed comments ignored), and forces `Include=0` for every lemma listed, on top of (never overriding back to `Include=1`) the automatic checks above. Use this for names the PROPN-ratio check misses - e.g. a name tagged `NOUN` more often than `PROPN` in this particular corpus, so it stays under the ratio threshold despite not being a real common noun. Recorded in its own `ManuallyExcluded` column so it's clear from the seed file which mechanism excluded a given lemma. Any listed lemma that doesn't match a current noun candidate is printed as unmatched (typo check) but otherwise ignored.

```
python3 from_tagged_corpus_to_seeds.py --proper-noun-ratio-threshold 0.99 --exclude-nouns-file extra_proper_nouns.txt
```
...
- `<out-dir>/confusion_words_*.csv` — same true-tag rows as `confusion_matrices.txt`, but with the `NOUN`/`VERB` prediction columns kept and the `OTHER` column broken out into `item-<pos>` summary columns instead of one column per individual word. Each counts the total number of TOKEN occurrences (not distinct word types) in that row that weren't predicted `NOUN`/`VERB`, bucketed by that word's own primary tag in the training corpus: `item-noun` if the word is one of `sorted_noun_tokens`, `item-verb` if one of `sorted_verb_tokens` (both always present, even if all-zero, for a stable schema across files), and otherwise `item-<pos>` for whatever that word's single most frequent corpus tag actually is (e.g. `item-adjective`, `item-determiner`, `item-auxiliary`, `item-pronoun`, `item-adverb`) - one extra column per part of speech actually present among that run's unpredicted words. There's no catch-all "neither" column - every real word has a primary tag, so it always lands in one of these. Punctuation is never classified in the first place (see Punctuation handling above), so it never shows up here either. This lets you see, per true tag, how many missed tokens were canonically nouns, verbs, adjectives, determiners, etc., without a sparse column per word.
...
- `<out-dir>/learned_patterns_*.xlsx` — one spreadsheet per run, the full trained model: every pattern learned from training and every filler (a specific corpus word, or `NOUN`/`VERB` if abstracted) it was ever seen with, unlike `pattern_usage_*.csv` which only covers patterns actually matched against a test-set word. Long/tidy format - one row per `(pattern, filler, count)` triple, sorted by pattern then by count descending - rather than one column per pattern, since the pattern space can run into the hundreds of thousands of columns for a full-size corpus, far past what a spreadsheet can show as a wide matrix. `count` is always > 0 (only observed pattern/filler pairs are included).
